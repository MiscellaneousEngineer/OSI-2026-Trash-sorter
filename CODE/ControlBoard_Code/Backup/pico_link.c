...
/* Map bin name -> enum. Returns BIN_UNKNOWN for unrecognized names. */
static bin_id_t bin_from_str(const char *s)
{
    if      (strcmp(s, "metal")   == 0) return BIN_METAL;
    else if (strcmp(s, "glass")   == 0) return BIN_GLASS;
    else if (strcmp(s, "plastic") == 0) return BIN_PLASTIC;
    else if (strcmp(s, "paper")   == 0) return BIN_PAPER;
    else if (strcmp(s, "other")   == 0) return BIN_OTHER;
    return BIN_UNKNOWN;
}
